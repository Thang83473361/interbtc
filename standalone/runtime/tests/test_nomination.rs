...
mod spec_based_tests {
    ...
    fn nomination_with_non_running_status_fails(status: StatusCode) {
        ...
        assert_noop!(
            Call::Nomination(NominationCall::withdraw_collateral(vault_id.clone(), 100))
                .dispatch(origin_of(account_of(ALICE))),
            SecurityError::ParachainNotRunning,
        );
    }
    ...
    fn integration_test_opt_out_postconditions() {
        ...
        test_with_nomination_enabled_and_vault_opted_in(|currency_id| {
            ...
            assert_eq!(nonce, 1);
            assert_eq!(
                VaultStakingPallet::compute_reward_at_index(nonce - 1, INTERBTC, &vault_id, &account_of(USER)).unwrap(),
