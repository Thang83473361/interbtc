...
fn withdraw_vault_global_pool_rewards(vault_id: &DefaultVaultId<Runtime>) -> i128 {
    ...
    assert_ok!(Call::Fee(FeeCall::withdraw_rewards(vault_id.clone())).dispatch(origin_of(vault_id.account_id.clone())));
    amount
}
...
fn withdraw_local_pool_rewards(vault_id: &DefaultVaultId<Runtime>, nominator_id: &AccountId) -> i128 {
    let amount =
        staking::StakingCurrencyAdapter::<Runtime, GetWrappedCurrencyId>::compute_reward(vault_id, nominator_id)
            .unwrap();
    assert_ok!(Call::Fee(FeeCall::withdraw_rewards(vault_id.clone())).dispatch(origin_of(nominator_id.clone())));
    amount
}
...
fn integration_test_fee_with_parachain_shutdown_fails() {
    test_with(|currency_id| {
        ...
        assert_noop!(
            Call::Fee(FeeCall::withdraw_rewards(vault_id_1.clone())).dispatch(origin_of(vault_id_1.account_id)),
