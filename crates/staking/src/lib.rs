...
impl<T: Config> Pallet<T> {
    ...
    pub fn force_refund(
        ...
        let reward = Self::withdraw_reward_at_index(nonce, currency_id, vault_id, &vault_id.account_id)?;
        let reward_as_fixed = SignedFixedPoint::<T>::checked_from_integer(reward).ok_or(Error::<T>::TryIntoIntError)?;
        ...
        Self::deposit_stake(currency_id, vault_id, &vault_id.account_id, reward_as_fixed)?;
...
pub trait Staking<VaultId, NominatorId> {
    /// Signed fixed point type.
...
impl<T, GetCurrencyId> Staking<DefaultVaultId<T>, T::AccountId> for StakingCurrencyAdapter<T, GetCurrencyId>
where
    T: Config,
    GetCurrencyId: Get<T::CurrencyId>,
{
    ...
    fn slash_stake(vault_id: &DefaultVaultId<T>, amount: Self::SignedFixedPoint) -> Result<(), DispatchError> {
